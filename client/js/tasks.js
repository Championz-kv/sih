...
const TASK_STATUS_OPTIONS = ['In progress', 'Not started', 'Completed', 'Due'];
const ADDED_TASKS = {};
const TASK_SEEDS = {};
...
/* Status stamp â€” loose class reuse so all four states keep the existing look. */
...
/* One row â€” seed or added â€” with an Edit button at the right side. */
...
function openAddTaskModal(){
  TASK_EDIT = null;
	...
	const st = document.getElementById('taskStatusSelect'); if(st) st.selectedIndex =  â€‹0;
...
function openEditTaskModal(idx){
  const key = currentTaskKey();
	...
	const st = document.getElementById('taskStatusSelect');
	if(st){ st.value = row[2] === 'Done' ? 'Completed' : row[2]; if(st.selectedIndex === -1 && st.options.length) st.selectedIndex =  â€‹0; }
	const sel = document.getElementById('taskOwnerSelect');
	if(sel){ sel.value = row[1]; if(sel.selectedIndex === -1 && sel.options.length) sel.selectedIndex =  â€‹0; }
...
function addTask(){
  const nameEl = document.getElementById('taskNameInput');
	...
	const due = dueRaw ? formatDate(dueRaw) : 'â€”';
